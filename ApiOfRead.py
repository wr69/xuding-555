...
def runapi(apilist,a):
    ...
    for b in range(len(apilist)):	
        if req.get(api_list[apilist[b]],headers=headers).status_code == 200:
            print(datetime.now().strftime("%Y-%m-%d %H:%M:%S"),'第'+str(apilist[b])+"号api调用成功")
        else:
            print(datetime.now().strftime("%Y-%m-%d %H:%M:%S"),"pass")
...
#实际运行
if int(app_num) > 1:
    print(datetime.now().strftime("%Y-%m-%d %H:%M:%S"),'多账户/应用模式下，日志报告里可能会出现一堆***，属于正常情况')
print(datetime.now().strftime("%Y-%m-%d %H:%M:%S"),"如果api数量少于规定值，则是api赋权没有弄好，或者是onedrive还没有初始化成功。前者请重新赋权并获取微软密钥替换，后者请稍等几天")
print(datetime.now().strftime("%Y-%m-%d %H:%M:%S"),'共 '+str(app_num)+r' 账号/应用，'+r'每个账号/应用 '+str(config['rounds'])+' 轮') 
for r in range(1,config['rounds']+1):
    ...
    for a in range(1, int(app_num)+1):
        ...
        client_secret=os.getenv('CLIENT_SECRET_'+str(a))
        print('\n\n')
        print(datetime.now().strftime("%Y-%m-%d %H:%M:%S"),'应用/账号 '+str(a)+' 的第'+str(r)+'轮 '+'\n')
        if config['api_rand'] == 1:
            print(datetime.now().strftime("%Y-%m-%d %H:%M:%S"),"已开启随机顺序,共十二个api,自己数")
            apilist=final_list
        else:
            print(datetime.now().strftime("%Y-%m-%d %H:%M:%S"),"原版顺序,共十个api,自己数")
